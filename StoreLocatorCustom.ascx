        ...
        <td class="column1" valign="top">        
            ...
            <ItemTemplate>
                <div style="padding: 10px 10px 10px 10px;">
                    <%# Eval("Address.Latitude")%> ,<%# Eval("Address.Longitude")%>)
                    <b><a href='javascript:showMap(<%# Eval("Address.Latitude")%>,<%# Eval("Address.Longitude")%>)'><%# Eval("Title")%></a></b>
                    <br />
                    <%# Eval("Address.Street")%>
                    <br />
                    <%# Eval("Address.City")%>, <%# Eval("Address.StateCode")%> <%# Eval("Address.Zip")%>
                    <br />
                    <span style='display:<%# Eval("Distance").ToString() == "0.00" ? "none" : "block"%>'>Distance: <%# Math.Truncate((double)Eval("Distance")) %> miles</span>
                </div>
            </ItemTemplate>
            </telerik:RadListView>
        </td>
